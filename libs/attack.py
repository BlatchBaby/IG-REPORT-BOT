...
def report_profile_attack(username, proxy):
    ...
    try:
        ...
        print_error("Connection error occurred!! (Facebook Requests Error)")
        return
    ...
    if ('["_js_datr","' not in res.text):
        print_error("Connection error occurred!! (Cookie Error JSDatr)")
        return
    
    try:
        js_datr = res.text.split('["_js_datr","')[1].split('",')[0]
    except:
        print_error("Connection error occurred!! (Cookie Parsing Error)")
        return
    ...
    try:
        ...
        print_error("Connection error occurred!! (Instagram Requests Error)")
        return
    ...
    if ('"__spin_r":' not in res.text):
        print_error("Connection error occurred!! (Cookie Error SpinR)")
        return

    if ('"__spin_b":' not in res.text):
        print_error("Connection error occurred!! (Cookie Error SpinB)")
        return

    if ('"__spin_t":' not in res.text):
        print_error("Connection error occurred!! (Cookie Error SpinT)")
        return

    if ('"server_revision":' not in res.text):
        print_error("Connection error occurred!! (Cookie Error Rev)")
        return

    if ('"hsi":' not in res.text):
        print_error("Connection error occurred!! (Cookie Error Hsi)")
        return
